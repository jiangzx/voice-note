...
定义语音记账交互 UI 的系统行为，包括语音状态动画、确认卡片、对话气泡、语音纠错、连续记账、消息类型系统、NLP 处理指示器、字段编辑、触觉反馈、动态快捷词、首次使用引导、手动模式按住说话与上滑取消、无障碍支持。
...
### Requirement: 确认卡片
系统 SHALL 在 CONFIRMING 状态展示交易确认卡片。卡片 SHALL 展示已解析的所有字段：金额、分类、日期、描述、类型、账户。每个字段 SHALL 可点击进入编辑。金额修改时 SHALL 以动画过渡更新。卡片 SHALL 以 slide-up + fade-in 动画入场。卡片 SHALL 展示识别来源徽标（Local/AI + 置信度指示器）。缺失字段（null 值）SHALL 以红色斜体高亮提示。
...
- **THEN** 系统 SHALL 在金额字段展示"点击输入金额"提示，该字段 SHALL 以红色斜体高亮标记
...
#### Scenario: 连续记两笔
- **WHEN** 用户说"午饭35" → 确认 → 系统提示"已记录 ¥35 → 餐饮" → 用户说"打车28" → 确认
- **THEN** 系统 SHALL 分别保存两笔交易，每笔确认后以 success 消息展示保存结果，并回到 LISTENING 状态
...
### Requirement: 无障碍支持
语音记账 UI SHALL 为所有交互组件提供 Semantics 语义标签。确认卡片、聊天气泡、录音控件、键盘发送按钮、模式切换控件、快捷词 Chip 等 SHALL 包含描述性 label。状态动画 SHALL 使用 liveRegion 向屏幕阅读器实时通报状态变化。
...
#### Scenario: 键盘发送按钮无障碍
- **WHEN** 屏幕阅读器聚焦到键盘模式的发送按钮
- **THEN** 系统 SHALL 朗读"发送"

#### Scenario: 模式切换控件无障碍
- **WHEN** 屏幕阅读器聚焦到输入模式切换控件
- **THEN** 系统 SHALL 朗读当前模式（如"输入模式：当前为自动"）

#### Scenario: 离线横幅无障碍
- **WHEN** 设备离线且屏幕阅读器活动
- **THEN** 离线横幅 SHALL 以 liveRegion 自动通报"当前离线，仅使用本地解析"

### Requirement: 手动模式按住说话按钮与上滑取消
在输入模式为「按住说话」时，系统 SHALL 提供按住说话按钮，支持按住录音、松开发送；并支持上滑进入取消区后松开取消本次识别（类似微信语音）。录音 SHALL 在按下后延迟启动（建议约 200ms），若在此时间内松开 SHALL 不启动录音、无任何提示与识别（点击即松视为误触无感）。若已启动录音但按住时长不足约定阈值（建议约 600ms）即松开且未在取消区，SHALL 取消本次录音并以 SnackBar 提示「说话时间太短」。手势 SHALL 以指针按下点为基准，垂直位移超过约定阈值（建议约 56 逻辑像素）判定为进入取消区；下滑回阈值内 SHALL 视为回到发送区（滑回发送）。录音中 SHALL 在按钮上方展示当前意图提示：「上滑取消」或「松开取消」（处于取消区时）。处于取消区时按钮 SHALL 展示取消图标（如 cancel_outlined），否则 SHALL 展示麦克风或波形。进入或离开取消区时 SHALL 触发 selectionClick 触觉反馈；松开时 SHALL 触发 lightImpact。

#### Scenario: 点击即松无感
- **WHEN** 用户按下录音按钮后于延迟（如 200ms）内即松开
- **THEN** 系统 SHALL NOT 启动录音，SHALL NOT 展示波形或提示，SHALL NOT 触发触觉反馈（或仅保留按下时反馈），无感处理

#### Scenario: 按住开始录音
- **WHEN** 用户在按住说话模式下按下录音按钮并保持超过约定延迟（如 200ms）
- **THEN** 系统 SHALL 开始录音并展示波形动画，按钮上方 SHALL 展示「上滑取消」

#### Scenario: 说话时间太短提示
- **WHEN** 用户已开始录音但按住时长不足约定阈值（如 600ms）即松开且未在取消区
- **THEN** 系统 SHALL 取消本次录音，SHALL 以浮动 SnackBar 展示「说话时间太短」

#### Scenario: 松开发送
- **WHEN** 用户在未进入取消区时松开
- **THEN** 系统 SHALL 结束本次录音并提交识别（pushEnd）

#### Scenario: 上滑进入取消区
- **WHEN** 用户按住后手指上滑超过阈值
- **THEN** 系统 SHALL 将当前意图置为取消区：提示文案 SHALL 变为「松开取消」，按钮 SHALL 展示取消图标，SHALL 触发 selectionClick

#### Scenario: 滑回发送区
- **WHEN** 用户在取消区内下滑回阈值以内
- **THEN** 系统 SHALL 将当前意图置为发送区：提示 SHALL 恢复「上滑取消」，按钮 SHALL 恢复波形/麦克风，SHALL 触发 selectionClick

#### Scenario: 松开取消
- **WHEN** 用户处于取消区时松开
- **THEN** 系统 SHALL 取消本次录音（pushCancel），不提交识别，状态 SHALL 回到 LISTENING

#### Scenario: 录音按钮无障碍
- **WHEN** 屏幕阅读器聚焦到按住说话按钮
- **THEN** 系统 SHALL 根据状态朗读「按住说话」或「松开 发送」或「松开 取消」（录音中且处于取消区时）
