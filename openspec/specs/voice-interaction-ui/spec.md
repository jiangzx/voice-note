...
在输入模式为「按住说话」时，系统 SHALL 提供按住说话按钮，支持按住录音、松开发送；并支持上滑进入取消区后松开取消本次识别（类似微信语音）。手势 SHALL 以指针按下点为基准，垂直位移超过约定阈值（建议约 56 逻辑像素）判定为进入取消区；下滑回阈值内 SHALL 视为回到发送区（滑回发送）。录音中 SHALL 在按钮上方展示当前意图提示：「上滑取消」或「松开取消」（处于取消区时）。处于取消区时按钮 SHALL 展示取消图标（如 cancel_outlined），否则 SHALL 展示麦克风或波形。进入或离开取消区时 SHALL 触发 selectionClick 触觉反馈；松开时 SHALL 触发 lightImpact。

#### Scenario: 按住开始录音
- **WHEN** 用户在按住说话模式下按下录音按钮
