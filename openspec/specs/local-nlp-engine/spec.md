...
定义客户端本地 NLP 规则引擎的系统行为，包括从自然语言输入中提取交易结构化信息（金额、日期、分类、类型）的规则和策略。
...
#### Scenario: 中文复杂数字金额
- **WHEN** 文本包含"两千五块"
- **THEN** 系统 SHALL 提取 amount=2500.0
- **NOTE**: 支持隐含位值（两千五 = 2500，一百二 = 120）

#### Scenario: 阿拉伯数字优先
- **WHEN** 文本同时包含阿拉伯数字和中文数字
- **THEN** 系统 SHALL 优先提取阿拉伯数字金额
