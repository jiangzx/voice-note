...
定义预算管理功能的系统行为，包括按分类设置月度预算、进度追踪、超支提醒、预算概览页、编辑页和自动继承逻辑。
...
### Requirement: 预算超支提醒
系统 SHALL 在消费达到预算 80% 和 100% 时通过本地通知提醒用户。每个分类每月每个阈值（80%/100%）最多 SHALL 触发一次通知。预算检查 SHALL 在交易保存后异步执行，使用当前年月作为检查维度。

> **实际行为说明**：编辑历史交易时，预算检查使用当前年月而非交易所属年月。此设计避免了回溯月份的复杂性，且编辑历史交易触发旧月预算通知的用户价值有限。
...
> **实际行为说明**：当前实现中继承在首次加载该月预算数据时执行（如进入预算概览页或预算编辑页时调用 `getOrInherit(yearMonth)`），保存交易后的预算检查（`checkAfterSave`）不触发继承。
