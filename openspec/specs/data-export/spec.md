...
定义数据导出功能的系统行为，包括 CSV/Excel 格式生成、筛选条件、文件命名、导出进度、文件分享和大数据量处理。
...
### Requirement: 导出筛选条件
系统 SHALL 支持按条件筛选导出内容。筛选条件 SHALL 包含：时间范围（起止日期）、交易类型（全部/支出/收入）。未设置筛选条件时 SHALL 导出全部数据。

> **实际行为**：当前导出选项 Sheet 仅提供时间范围与交易类型筛选，不包含分类、账户多选。
