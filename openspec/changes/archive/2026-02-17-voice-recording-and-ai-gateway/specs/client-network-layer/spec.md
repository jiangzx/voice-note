...
系统 SHALL 提供 Server 连接配置管理，包含 Base URL 和超时时间。配置 SHALL 支持通过设置页修改（高级设置）。默认配置 SHALL 指向生产环境。

#### Scenario: 默认开发配置
- **WHEN** App 首次启动
- **THEN** 系统 SHALL 使用预设的默认 Base URL（开发阶段为 `http://localhost:8080`，正式发布前更新为生产地址）
