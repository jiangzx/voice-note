...
- [x] 去掉 Flutter 侧音频会话/TTS 兜底依赖，语音会话统一走 `NativeAudioGateway`，避免双链路并发。
- [x] Android 先行：新增原生 ASR WebSocket 传输（Flutter 仅下发 token/ws/model），采集与上行改由 Android runtime 承担。
- [x] iOS 对等实现原生 ASR 传输，移除 Flutter 侧 `AudioCaptureService/VAD` 依赖。
- [ ] 补齐 “native-only” 回归用例（会话启动失败、TTS 超时、barge-in 时序）。
