...
单次语音可包含**单笔或多笔**交易（如「吃饭60，奶茶15」），系统解析后进入确认态；多笔时支持逐条确认/取消、语音纠正（如「第三笔改成支出」「红包那笔改为收入」），纠正请求优先走服务端 LLM，超时或离线时降级为本地规则。
...
| 方式 | 触发 | 说明 |
|---|---|---|
| **语音纠错** | 说"不对 / 改一下 / 金额改成XX / 分类改成XX" | 优先调用服务端 LLM 纠正接口（batch 上下文 + 纠正文本）；超时或离线时降级为本地规则 |
| **多笔定点纠正** | 说「第N笔改成XX」「红包那笔改为收入」 | 同上，LLM 根据 batch 与序号/描述词定位后返回 corrections |
| **UI 纠错** | 点击确认卡片上的任意字段 | 弹出编辑面板，支持手动修改 |
| **整条撤销** | 说"不要了 / 取消 / 删掉" | 取消当前这条记录（多笔时为清空当前 batch），回到监听态 |
| **逐条取消** | 说「删掉第二笔」 | 多笔确认态下仅取消指定的一笔，其余保留 |
...
| 触发条件 | 行为 |
|---|---|
| 静默 **2 分 30 秒** | TTS 提示：「还在吗？暂时不用的话我会先休息哦，30秒后自动退出」 |
